...
(function() {
  ...
  var acceptInnerOp = false;

  var Listening = function(state) {
    this.state = state;
    state.message = state.messages.LISTENING;
    this.advanceConversation = function() {
      if(state.lexConfig.contentType == 'text/plain; charset=utf-8'){
        state.transition(new Initial(state));
        acceptInnerOp = true;
      }
      if(acceptInnerOp && state.inputStream != null && !(state.inputStream instanceof Blob)){
        //state.inputStream = text;
        console.log("SEND ME TEXT!");
      }
      if(state.lexConfig.contentType == 'audio/x-l16; sample-rate=16000'){
        console.log(state.lexConfig.contentType);
  ...
  var Sending = function(state) {
    ...
    this.advanceConversation = function() {
      ...
      console.log(JSON.stringify(state.lexConfig));
